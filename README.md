...
Могут существовать различные view модели опиывающие один и тот же формат XML, что позволяет донастраивать отображение сохраная структуру данных XML.
...
- **elements** : object - элементы
  - **[id]** : object - элемент. **id** - уникальный идентификатор элемента (в списке элементов)
    ...
    - **repeatable** : object - описание повторяемости элемента
      - **path** : string - путь к повторяющимуся узлу дерева XML
...
[view]: ### view - модель данных

view модель описывает структуру модели данных, способ преобразования данных из JSON в XML и наоборот, испособ отображения данных.
...
[view/elements]: ### elements - элементы

Перечень элементов, обладающих уникальными (в рамках view) идентификаторами.

[view/element]: ### element - элемент
Елемент описывает узед в модели данных.
...
### repeatable - описание повторяемости элемента
...
### restriction - описание ограничений
...
Валидация документа выполняется по элементно, в эжтой связи идентификатором валидатора является ссылка на проверяемый элемент.
